...
from utils import load_target_audio, inverse_softplus, inverse_sigmoid, inverse_tanh
...
def main():
    ...
    target_ir = target_ir.to(torch.float32)  # Ensure double precision for stability
    ...
    checkpoint_path = os.path.join(checkpoint_dir, "train_progress.npz")
    ...
    for iteration in range(num_iterations):
        ...
        # Cache current parameters/loss to history each iter
        ...
        history['D_over_mu'].append(D_over_mu)
        history['T0_over_mu'].append(T0_over_mu)
        history['Ly'].append(Ly)
        history['xo'].append(xo)
        history['yo'].append(yo)
        history['loss'].append(loss_val)
        for name, param in model.named_parameters():
            if param.requires_grad and param.grad is not None:
                # param.grad is a tensor of the exact same shape as param
                gradient_residual = param.grad.clone().detach()
                if(f'res_{name}'== 'Ly_raw'):
                    history['res_Ly'].append(gradient_residual.cpu().numpy());
                elif(f'res_{name}'== 'xo_raw'):
                    history['res_xo'].append(gradient_residual.cpu().numpy());
                elif(f'res_{name}'== 'yo_raw'):
                    history['res_yo'].append(gradient_residual.cpu().numpy());
                elif(f'res_{name}'== 'mu_raw'):
                    history['res_mu'].append(gradient_residual.cpu().numpy());
                elif(f'res_{name}'== 'D_over_mu_raw'):
                    history['res_D_over_mu'].append(gradient_residual.cpu().numpy());
                elif(f'res_{name}'== 'T0_over_mu_raw'):
                    history['res_T0_over_mu'].append(gradient_residual.cpu().numpy());

        # Periodically flush progress to disk, so mid run quit/killed is recoverable
        if iteration % 10 == 0 or iteration == num_iterations - 1:
            np.savez(checkpoint_path, **history)

            print(f"Ly: {Ly:.4f}m | xo: {xo:.4f}m | yo: {yo:.4f}m | "
                  f"mu: {mu:.4f} | D/mu: {D_over_mu:.2f} | T0/mu: {T0_over_mu:.4f}")
            # Iteration and loss logs
            print(f"Iteration {iteration:04d} | Loss: {loss_val:.6f}")
            

    total_time = time.time() - start_time
    print(f"\nOptimization complete in {total_time:.2f} seconds.")

    # Save final checkpoint and model weights
    np.savez(checkpoint_path, **history)
